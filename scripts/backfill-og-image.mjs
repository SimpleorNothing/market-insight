// 기존 news.json 백필: image 필드가 없는 레코드에 원문 대표 이미지(og:image)를
// 소급 부착한다. 신규 수집분은 fetch-news.js(enrichImages)가 자동으로 채우므로,
// 이 스크립트는 도입 시점 이전 축적분(약 630건)을 1회 정정하는 용도다.
//
// - LLM 호출 아님 — 실기사 HTML fetch + 정규식(og:image/twitter:image). 토큰 비용 0.
...
//     --dry   : 파일 미기록, 통계만 출력
//     --force : 이미 image 키가 있어도 재수집(기본은 image 키 없는 건만)
...
// scripts/fetch-news.js 의 fetchOgImage 와 동일 로직(추출 결과 일관성 유지)
...
// 대상: skip 제외 + (force면 전부 / 기본은 image 키 미보유분만)
const targets = all.filter(
  (n) => n.lens !== "skip" && (FORCE || !("image" in n))
);

console.log(
  `대상 ${targets.length} / 전체 ${all.length}건` +
    (FORCE ? " (--force: image 키 유무 무시)" : " (image 키 없는 건만)")
);
if (!targets.length) {
...
console.log(`og:image 확보: ${hit}/${targets.length}건 (나머지는 null → 색 플레이스홀더)`);
